...
export function ExamineOverlay({ onClose, onDispatch }: Props) {
  ...
  const tabs: Array<{ id: Tab; label: string; badge?: number | string; disabled?: boolean }> = [
    ...
    { id: 'diagnose', label: '判定' },
    { id: 'rx', label: '处置', badge: rxCount > 0 ? rxCount : undefined, disabled: !rxUnlocked },
  ];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span>按 Esc 关闭 · 已开 {ordered.size} 项化验</span>
          <span>{submitted ? `诊断：${diagLabel(submitted)}` : '尚未提交诊断'}</span>
        </div>
      </div>
    </div>
  );
...
function HistoryTab({ patient }: { patient: NonNullable<ReturnType<typeof useGameState>['polyclinic']['patient']> }) {
  ...
  if (c.anamnesis.length === 0) {
    return <div style={{ color: 'var(--ink-2)', fontWeight: 700 }}>本病例无可问诊条目。</div>;
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {answered.map((q) => (
        ...
        >
          <div style={{ fontWeight: 800, fontSize: 13, color: 'var(--ink-2)' }}>你问过</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-2)' }}>
        综合门诊化验 <strong>即时</strong> 返回。各分组默认折叠 —— 点击行展开。
      </div>
      ...
      {visiblePanels.length > 0 && (
        <CollapsibleSection
          icon="🧪"
          label={`${currentClinic === 'all-specialties' ? '各专科通用' : '本科室'}化验组合`}
      ...
      {(['bedside', 'lab', 'imaging'] as const).map((cat) => {
        ...
        const label = cat === 'bedside' ? '床旁' : cat === 'lab' ? '化验室' : '影像';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {submitted && (
        ...
        >
          {isCorrect
            ? `✓ 判定正确 — ${diagLabel(c.correctDiagnosisId)}。`
            : `✗ 判定有误。正确应为 ${diagLabel(c.correctDiagnosisId)}。`}
        </div>
      )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {specialty && specialty !== 'all-specialties' && (
        ...
        >
          范围：<strong>{CLINIC_LABELS[specialty]}</strong> 药品目录 ——
          本专科以外的药物已隐藏。
        </div>
      )}
